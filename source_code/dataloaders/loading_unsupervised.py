...
class Unsupervised_Dataset_Loader(Dataset):

    def __init__(self, root_dir, size, scale, crop_size_H, crop_size_W, transform=None, random_crop=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.random_crop:
            assert w > self.scale*tw and h > self.scale*th, 'cropped size larger than the image size'
            random.seed()
            h1 = random.randint(0, h - th)
            w1 = random.randint(0, w - tw)

            h2 = random.randint(0, h - self.scale*th)
            w2 = random.randint(0, w - self.scale*tw)
            ...
            while(is_low_contrast(img_c, h2, w2, self.scale*th, self.scale*tw, 8)):
                h2 = random.randint(0, h - self.scale*th)
                w2 = random.randint(0, w - self.scale*tw)
        ...
        img_adv_cen = img_c[h2: h2 + self.scale*th, w2: w2 + self.scale*tw, :]
        img_adv_cen = torch.from_numpy(img_adv_cen).float()
        img_adv_cen = img_adv_cen.permute(2, 0, 1)

        img_adv_ref = img_others_np[:, h2: h2 + self.scale*th, w2: w2 + self.scale*tw, :]
